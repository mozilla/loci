...
const {normalizeUrl} = require("lib/task-queue/utils");
...
exports["test url normalization"] = function(assert) {
  assert.equal(normalizeUrl(INVALID_URL), undefined, "Invalid url returns undefined");

  for (let urls of URL_GROUPS) {
    const normalized = urls.map((url)=> normalizeUrl(url));
    assert.ok(normalized.every((curr, i, array)=> {
      return curr === urls[0];
    }));
  }
};
